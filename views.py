...
@login_required
def map():
    return render_template('map.html', key=APP.config['KEY'])
...
def post():
    ...
    if form.validate_on_submit():
        ...
        db.execute("INSERT INTO post('post','title','location','id') values(:post, :title, :location, :id)", post=post, title=title,location = location, id=session.get("id"))
        form.title.data=''
        form.post.data=''
        form.location.data=''
